...
def processInputDir(input_dir, output_dir):
    for path in Path(input_dir).glob("*.html"):
        ...
        for ch in inp.body.find_all(True):
            div, styled_components = process(ch, matchers, outp, div, styled_components)
            if styled_components not in total_styled_components:
                total_styled_components += styled_components

        outp.append(div)

        writeReactNativeFile(output_dir, path.name, total_styled_components, outp)
...
def writeReactNativeFile(output_dir, filename, total_styled_components, outp):
    ...
    content = template.substitute(
        styled_components=total_styled_components, jsx=outp.prettify(formatter="html")
    )
...
if __name__ == "__main__":
    ...
    matchers = [
        Matcher(
            "a", "class", "w-button", generators.gen_button, input_dir, output_dir, 0
        ),
        Matcher("a", "class", "link", generators.gen_button, input_dir, output_dir, 0),
        Matcher(
            "div", "class", "w-container", generators.gen_view, input_dir, output_dir, 0
        ),
        Matcher(
            "div", "class", "w-form", generators.gen_view, input_dir, output_dir, 0
        ),
        Matcher(
            "input",
            "class",
            "w-input",
            generators.gen_textinput,
            input_dir,
            output_dir,
            0,
        ),
        ...
        Matcher(
            "h1", "class", "heading", generators.gen_text, input_dir, output_dir, 0
        ),
    ]
